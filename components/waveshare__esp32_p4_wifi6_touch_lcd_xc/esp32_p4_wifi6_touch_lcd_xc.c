...
static esp_err_t bsp_touch_hw_reset(void)
{
...
esp_err_t bsp_touch_new(const bsp_display_cfg_t *cfg, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    for (int attempt = 0; attempt < 5; ++attempt) {
        esp_err_t reset_ret = bsp_touch_hw_reset();
        if (reset_ret != ESP_OK) {
            last_error = reset_ret;
        }

        for (size_t i = 0; i < sizeof(touch_addresses) / sizeof(touch_addresses[0]); ++i) {
            uint8_t address = touch_addresses[i];
            if (bsp_i2c_device_probe(address) != ESP_OK) {
                continue;
            }

            ESP_LOGI(TAG, "Touch 0x%02x found on attempt %d", address, attempt + 1);
            esp_lcd_panel_io_i2c_config_t tp_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
            tp_io_config.dev_addr = address;
            tp_io_config.scl_speed_hz = CONFIG_BSP_I2C_CLK_SPEED_HZ;

            esp_lcd_panel_io_handle_t tp_io_handle = NULL;
            last_error = esp_lcd_new_panel_io_i2c(i2c_handle, &tp_io_config, &tp_io_handle);
            if (last_error != ESP_OK) {
                ESP_LOGW(TAG, "Touch IO init failed on 0x%02x: %s", address, esp_err_to_name(last_error));
                continue;
            }

            last_error = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, ret_touch);
            if (last_error == ESP_OK) {
                return ESP_OK;
            }

            ESP_LOGW(TAG, "Touch init failed on 0x%02x attempt %d: %s",
                     address, attempt + 1, esp_err_to_name(last_error));
    ...
    ESP_LOGE(TAG, "Touch not initialized after retries: %s", esp_err_to_name(last_error));
    return last_error;
}
...
static lv_indev_t *bsp_display_indev_init(const bsp_display_cfg_t *cfg, lv_display_t *disp)
{
    ...
    if (ret != ESP_OK) {
        tp = NULL;
        ESP_LOGW(TAG, "Touch init failed (%s), continuing without touch input", esp_err_to_name(ret));
        return NULL;
    }
...
lv_display_t *bsp_display_start_with_config(bsp_display_cfg_t *cfg)
{
    ...
    if (!disp_indev) {
        ESP_LOGW(TAG, "Display input device disabled");
    }
